...
def test_execute_case_from_args_runs_network_verify_before_kill_wait(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "run_ems_login_check", lambda: calls.append(("ems-login",)) or {"enabled": True, "skipped": False, "status": "ok", "storage_state": "state.json", "url": "https://ems/"})
    ...
    assert result["ems_login_result"]["status"] == "ok"
...
def test_execute_case_continues_when_optional_ems_login_check_fails(tmp_path, monkeypatch):
    case_path = tmp_path / "case.yaml"
    case_path.write_text(
        "\n".join(
            [
    ...
    monkeypatch.setattr(config, "DEFAULT_WAIT_SECONDS", 25)
    monkeypatch.setattr(config, "DELETE_WORKFLOW_AFTER", True)
    monkeypatch.setattr(config, "EMS_LOGIN_INTERACTIVE_ON_FAILURE", False)
    monkeypatch.setattr(config, "EMS_LOGIN_REQUIRED", False)
    monkeypatch.setattr(config, "to_dict", lambda mask_secrets=False: {"ACTIVE_CONFIG_FILE": ""})
    monkeypatch.setattr(
        runner,
        "build",
    ...
    monkeypatch.setattr(runner, "_render_phase_yaml", lambda case, resolved: ("", None))
    monkeypatch.setattr(runner, "expand_network_chaos_to_component_pods", lambda text, ns: text)
    monkeypatch.setattr(runner, "run_ems_login_check", lambda: (_ for _ in ()).throw(RuntimeError("expired")))
    monkeypatch.setattr(runner, "run_workflow", lambda path, wf_ns, wf_name, wait_seconds, cleanup=True, during_wait=None, yaml_text=None: calls.append(("run", wf_name)) or {"execution_mode": "local", "yaml_path": path})
    monkeypatch.setattr(runner, "run_ems_alarm_query", lambda *args, **kwargs: calls.append(("ems",)) or {})
    ...
    assert calls == [("run", "wf-single")]
    assert result["ems_login_result"]["error"] == "expired"
    assert result["ems_alarm_result"]["skipped"] is True


def test_execute_case_opens_manual_ems_login_when_check_fails(tmp_path, monkeypatch):
    case_path = tmp_path / "case.yaml"
    case_path.write_text(
        "\n".join(
            [
    ...
    monkeypatch.setattr(config, "DEFAULT_WAIT_SECONDS", 25)
    monkeypatch.setattr(config, "DELETE_WORKFLOW_AFTER", True)
    monkeypatch.setattr(config, "EMS_LOGIN_INTERACTIVE_ON_FAILURE", True)
    monkeypatch.setattr(config, "to_dict", lambda mask_secrets=False: {"ACTIVE_CONFIG_FILE": ""})
    monkeypatch.setattr(
        runner,
        "build",
        lambda case, cfg: (
            "\n".join(["apiVersion: chaos-mesh.org/v1alpha1", "kind: Workflow", "spec:", "  templates: []"]),
            {},
        ),
    )
    monkeypatch.setattr(runner, "_render_phase_yaml", lambda case, resolved: ("", None))
    ...
    def fake_check():
        ...
        calls.append(("ems-check",))
        return {"enabled": True, "skipped": False, "status": "ok", "storage_state": "state.json", "url": "https://ems/"}

    monkeypatch.setattr(runner, "run_ems_login_check", fake_check)
    monkeypatch.setattr(runner, "run_ems_manual_login", lambda: calls.append(("ems-manual",)) or {"rc": 0})
    monkeypatch.setattr(runner, "run_workflow", lambda path, wf_ns, wf_name, wait_seconds, cleanup=True, during_wait=None, yaml_text=None: calls.append(("run", wf_name)) or {"execution_mode": "local", "yaml_path": path})
    monkeypatch.setattr(runner, "run_ems_alarm_query", lambda *args, **kwargs: calls.append(("ems-alarm",)) or {"enabled": True, "skipped": True, "reason": "ok"})
    ...
    assert calls[:3] == [("ems-manual",), ("ems-check",), ("run", "wf-single")]
    assert result["ems_login_result"]["status"] == "ok"
...
def test_execute_case_from_args_does_not_cleanup_network_when_verify_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "_delete_workflow_once", lambda wf_ns, wf_name: calls.append(("cleanup", wf_name)) or "deleted")
    monkeypatch.setattr(runner, "run_ems_login_check", lambda: calls.append(("ems-login",)) or {"enabled": True, "skipped": False, "status": "ok"})
    ...
    assert calls == [("ems-login",), ("apply", "wf-phased-net")]
